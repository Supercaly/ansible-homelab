...
This role is responsible for preparing the filesystem layout, deploying Compose files and templates, and executing Docker Compose operations (pull, prune, cleanup).
...
| `docker_compose_default_owner` | | string | `"root"` | Default owner for created files and directories. |
| `docker_compose_default_group` | | string | `"root"` | Default group for created files and directories. |
| `docker_compose_default_mode`  | | string | `"0755"` | Default permissions for created directories. |
...
| `docker_compose_prune` | | bool | `true` | Remove unused images after deployment. |
