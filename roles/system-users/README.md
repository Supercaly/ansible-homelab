...
|`system_users_default_shell` | | string | `"/bin/bash"` | Default shell used when creating users.|
|`system_users_sudo_includedir`| | string | `"@includedir /etc/sudoers.d"` | Command used in `etc/sudoers` to include external files.|
...
|`sudo_options`| | list\|string | | List of options for the group appended to the sudoers file. |
...
|`sudo_options`| | list\|string | | List of options for the user appended to the sudoers file. |
