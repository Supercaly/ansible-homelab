...
|`scsi`| | object | | Dictionary of SCSI disks definitions. Keys must be scsi[n]. Values are dictionaries with keys (storage, size, format, backup, discard, iothread, ssd). `size` must be a plain integer in GiB (no unit suffix). |
...
This role depends on the collection `community.proxmox` for managing Proxmox KVMs.
